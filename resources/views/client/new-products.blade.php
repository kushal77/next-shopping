...
<section class="breadcrumbs py-4">
    <div class="container-fluid px-5">
        <h6><a href="{{route('home')}}">Home</a><i class="fas red mx-2 fa-chevron-right"></i>New Products</h6>
        <hr class="mt-4">
    </div>
</section>
        ...
        <div class="row">
            ...
            <div class="col-lg-10 right-panel">
                <div class="sort-by pt-5 px-5 d-flex flex-wrap align-items-center justify-content-between">
                    <div class="heading">
                        <h4 class="font-weight-bold">Newest Products</h4>
                        <p class="my-3 text-muted">{{count($newproducts)}} items found in Newest Products</p>
                    </div>
                ...
                <div class="row newest-product py-5 px-5">
                    ...
                    @foreach($newproducts as $new)
                        <div class="col-lg-4 col-md-6 py-3">
                            <div class="card">
                                <a href="{{route('view.product',$new->alias)}}" class="card-img">
                                    <div class="img-box p-3">
                                        <img src="{{asset('images/product/'.$new->images[0]->image)}}">
                                    </div>
                                    <div class="cart-detail">
                                        <a href="{{route('view.product',$new->alias)}}" class="button details text-white my-2">View Details</a>
                                        <a class="button cart text-white my-2 addtocart" data-item="{{encrypt($new->id)}}">Add to Cart</a>
                                    </div>
                                </a>
                                <div class="card-text p-4" data-mh="eq">
                                    <h6 class="font-weight-bold"><a href="{{route('view.product',$new->alias)}}" data-toggle="tooltip" title="{{$new->title}}">{{str_limit($new->title,50)}}</a></h6>
                                    @if($new->discount > 0)
                                        <h5 class="price red d-inline-block my-2 mr-3">{{$new->currency}}. {{$new->net_price}}</h5>
                                        <h5 class="text-muted d-inline-block"><strike>{{$new->currency}}. {{$new->price}}</strike></h5> 
                                    @else
                                        <h5 class="price red d-inline-block my-2 mr-3">{{$new->currency}}. {{$new->net_price}}</h5>
                                    @endif
                                </div>
                    ...
                    @endforeach
                    {{ $newproducts->links('client.pagination')}}
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script type="text/javascript">
        $('#rightsearchblock').on('submit',function(e) {
            ...
            window.location.href='{{ url('/new-products') }}?brand='+brands.toString()+'&&category='+category.toString()+'&&min='+min+'&&max='+max;
        })
        ...
        $('#choosetype').on('change',function(e){
            ...
            window.location.href='{{ url('/new-products') }}?brand='+brands.toString()+'&&category='+category.toString()+'&&min='+min+'&&max='+max+'&&choosetype='+choosetype;
        })
